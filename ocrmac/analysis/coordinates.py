    ...
    def _map_coordinates(
        ...
        Args:
            bbox: [x, y, w, h] 切片内归一化坐标
                  注意：Vision 坐标系 y=0 在底部，y=1 在顶部
            slice_y_start: 切片在原图中的起始 y (像素，从顶部算起)
            slice_y_end: 切片在原图中的结束 y (像素，从顶部算起)

        Returns:
            [x, y, w, h] 原图归一化坐标（转换为 y=0 在顶部）
        """
        x, y, w, h = bbox
        slice_height = slice_y_end - slice_y_start

        # Vision 坐标系：y 是文本框底部距离图片底部的比例
        # 转换为从顶部算起的像素坐标：
        # y_from_top = (1 - y - h) * slice_height  (文本框顶部距离切片顶部)
        y_from_top_in_slice = (1 - y - h) * slice_height
        h_abs = h * slice_height

        # 映射到原图的绝对坐标（从顶部算起）
        y_abs = slice_y_start + y_from_top_in_slice
...
class TextMerger:
    ...
    def merge_to_text(results: List[MergedResult], preserve_lines: bool = True) -> str:
        ...
        # 每行内按 x 坐标排序，但如果 x 间隔太大则分开
        text_lines = []
        for line_results in lines:
            sorted_line = sorted(line_results, key=lambda r: r.bbox[0])
            # 检查是否有大的 x 间隔（可能是不同列）
            # 使用较小的阈值避免合并不同列的文本
            segments = TextMerger._split_by_x_gap(sorted_line, max_gap=0.05)
            for segment in segments:
                line_text = " ".join(r.text for r in segment)
    ...
    def _split_by_x_gap(
        ...
        Args:
            results: 已按 x 排序的结果列表
            max_gap: 最大允许的 x 间隔（归一化）
    ...
    def _group_by_lines(
        ...
        Args:
            results: OCR 结果列表
            y_tolerance: y 坐标容忍度（会根据文本高度动态调整）
        ...
        for result in sorted_results[1:]:
            # 使用文本高度的一半作为动态容差
            # 这比固定的 y_tolerance 更准确
            avg_height = (result.bbox[3] + current_line[0].bbox[3]) / 2
            dynamic_tolerance = min(y_tolerance, avg_height * 0.5)

            y_center = result.bbox[1] + result.bbox[3] / 2
            current_y_center = current_y + current_line[0].bbox[3] / 2

            if abs(y_center - current_y_center) <= dynamic_tolerance:
